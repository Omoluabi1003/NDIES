...
async function main(){
  ...
  await prisma.diasporaProfile.createMany({data: profiles.map(({id,...p})=>p)});
  await prisma.engagementProgram.createMany({data: programs.map(({id,...p})=>p)});
  await prisma.diasporaOrganization.createMany({data:[
...
main().then(async()=>{await prisma.$disconnect(); console.log("Seeded NDIES sample data")}).catch(async(e)=>{console.error(e); await prisma.$disconnect(); process.exit(1)});
